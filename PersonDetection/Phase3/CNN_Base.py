...
def conv2d(input_, output_dim, k_h, k_w, d_h, d_w, stddev=0.02, name="conv2d"):    
  with tf.variable_scope(name):
    ...
    biases = tf.get_variable('biases', [output_dim], initializer=tf.truncated_normal_initializer(stddev=stddev))
    conv = tf.reshape(tf.nn.bias_add(conv, biases), conv.get_shape())
    return conv

def deconv2d(input_, output_shape, k_h, k_w, d_h, d_w, stddev=0.02, name="deconv2d", with_w=False):  
  with tf.variable_scope(name):
  ...
  with tf.variable_scope(scope or "Linear"):
    try:
      matrix = tf.get_variable("Matrix", [shape[1], output_size], tf.float32,
        ...
        msg = "The output dimention is wrong"
