"""
Created by Bing Liu
Create dataset used to train amplitude CNN model
"""
...
def Get_Data_File_List(IQ_Data_File_Path, Training_File_Location_Filters):
    files_list = []
    data_files = com.Get_File_name_List(IQ_Data_File_Path)
    for file in data_files:
       location_code = Get_Location_Code(file, IQ_Data_File_Path)
...
def Get_Location_Code(file_name, IQ_Data_File_Path):
    index_S = len(IQ_Data_File_Path) + len(hd.IQ_Data_File_Name_Prefix) + len(hd.Raw_Data_File_Name_Start_Index) + 1
...
def CreateDataSet_Column(Raw_file_list, Reqs_Num):
    ...
    for raw_file in Raw_file_list:
        ...
        i = 0
        amp_pack = np.zeros((len(Raw_arr), IQ_Num, 1)).astype(hd.IQ_Data_Type_Str)

        for raw in Raw_arr:
            I, Q = np.hsplit(raw, [1])
            amp_pack[i] = np.sqrt(np.square(I) + np.square(Q))
            i = i + 1

        Lables.append(lable_arr[0])
        AMP_All.append(amp_pack)

    return Lables, AMP_All


def DumpToFile(Raw_file_list, RawDataSet, Reqs_Num):
    print("Dump " + str(len(Raw_file_list)) + " files to : " + RawDataSet)
    Lables, Values = CreateDataSet_Column(shuffle(Raw_file_list), Reqs_Num)
    with open(RawDataSet, 'wb') as f:
        pickle.dump((Lables, Values), f, pickle.HIGHEST_PROTOCOL)


def Create(IQ_Data_Path, 
           Data_Set_Path, 
           Train, 
           Valid, 
           Test, 
    ...
    file_list_all = []
    file_list_all = Get_Data_File_List(IQ_Data_Path, Location_Filters)
