...
import AMPD
import PowerChartPick

window=10

#Caculate the powers of frequency bands
def Get_Freqs_Powers(raw_file_list, IS_Active = True):
    ...
    for raw_file in raw_file_list:
        ...
        if IS_Active == True:
            ...
            db = 0 - 10*np.log10(power_per_file/len(raw_data_per_file[0])/2)
...
if __name__ == '__main__':
    ...
    freqs, power_arry = Get_Freqs_Powers(file_list, IS_Active = False)
    ...
    title += name + '\n'
    PowerChartPick.Display(freqs, power_mean_all, picked, title, IS_Active = False )
