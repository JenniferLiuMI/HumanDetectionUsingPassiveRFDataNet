"""
Created by Bing Liu
Human detection FFT amplitude CNN model
"""
import pickle
import numpy as np
...
class CNNNet:
    channel = 1
    n_classes = 2
    x = tf.placeholder(hd.Raw_Data_Type, (None, 0, 0, channel))
    y = tf.placeholder(tf.int32, (None))
    keep_prob = tf.placeholder(hd.Raw_Data_Type)       # For fully-connected layers
    keep_prob_conv = tf.placeholder(hd.Raw_Data_Type)  # For convolutional layers
