...
#Plot the powers
def Display(freqs, powers, peaks_list, title="", IS_Active = True):
    ...
    plt.subplots_adjust(left=0.02, right=0.98, top=0.99, bottom=0.075)

    line1, = ax1.plot(x, y,'-k', color='gray', linewidth=0.5, picker=3)
    ...
    for peak in peaks_list:
        if peak == 1:
            insert_xy(x_buy, freqs[i])
            #ax1.annotate(str(freqs[i]), xy=(freqs[i],float(powers[i])), color='red', arrowprops=dict(facecolor='red', width = 6, headwidth=6, headlength=6))
            ax1.annotate(" ", xy=(freqs[i],float(powers[i])), color='red', arrowprops=dict(facecolor='red', width = 6, headwidth=6, headlength=6))
        i += 1

    def remove_annotation_this(xy):
    ...
    def on_pick(event):
        ...
        #buy is red, sell is green
        color_action = 'red'

        for i,j in zip(xdata[ind:ind+1],ydata[ind:ind+1]):
            xy = (i,j)
            if check_annotation_exist(xy):
                remove_annotation_this(xy)
            else:
                insert_xy(x_buy, i)
                #annotation = ax1.annotate(str(i), xy=(i,j), color=color_action, arrowprops=dict(facecolor='red', width = 6, headwidth=6, headlength=6))
                annotation = ax1.annotate(" ", xy=(i,j), color=color_action, arrowprops=dict(facecolor='red', width = 6, headwidth=6, headlength=6))

    def key_press(event):
        if event.key == 'f4':
            np.savetxt(hd.Root +  'RFRawData/FreqsList/freq1.txt', x_buy, delimiter=',', fmt='%d')
            print("Total " + str(len(x_buy)) + " freqs are outputted into file freq.txt")
...
#Insert X Y values
def insert_xy(xy_list, xy_insert):
    if len(xy_list) == 0:
        xy_list.insert(0, xy_insert)
        return xy_list       
    i = 0
    for xy in xy_list:
        if xy_insert > xy:
